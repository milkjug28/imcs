...
export async function getCollectionStats(): Promise<CollectionStats | null> {
  ...
  try {
    const [listingsData, salesData] = await Promise.all([
      fetchJSON(`https://api.opensea.io/api/v2/listings/collection/${COLLECTION_SLUG}/all?limit=50`),
      fetchJSON(`https://api.opensea.io/api/v2/events/collection/${COLLECTION_SLUG}?event_type=sale&limit=50`),
    ])
    ...
    // Floor price from lowest listing
    let floorPrice = 0
    ...
    // Find whales (buyers with multiple purchases)
    ...
    // Build summary string for bot context
    const parts: string[] = []
    ...
    console.error('[opensea-stats] fetch failed:', err)
    return cached
  }
}
